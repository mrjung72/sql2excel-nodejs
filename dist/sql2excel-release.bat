...
if not exist "sql2excel.exe" (
    echo sql2excel.exe file not found.
...
if "%file_type%"=="xml" (
    sql2excel.exe validate --xml "%query_file%"
) else (
    sql2excel.exe validate --query "%query_file%"
)
...
sql2excel.exe export --xml "%xml_file%"
...
sql2excel.exe export --query "%json_file%"
